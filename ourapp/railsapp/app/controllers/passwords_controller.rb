class PasswordsController < ApplicationController
  ...
  def find_password
    password = Password.find_by(test_user_id: params[:test_user_id])

    if password
      render json: password
    else
      render json: {error: 'Password not found'}, status: :not_found
    end
  end
  ...
  private
    ...
    def password_params
      params.require(:password).permit(:test_user_id, :hashed_password)
    end
end
